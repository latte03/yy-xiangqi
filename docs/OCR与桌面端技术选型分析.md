...
| 分类模型 | **小型 CNN/ONNX**（多背景合成训练）；模板匹配作首版验证 | 棋盘半透明、底色随场景变，CNN 比纯模板更稳；单皮肤下合成数据可无限生成 |
...
> - **游戏截图**：可做到近 100% 准确，模板匹配即可，几乎不需要训练模型。
...
截图输入
  ...
  │     - 方案A 模板匹配：与 14 种棋子模板做相似度比对（起步快）
  │     - 方案B 小型 CNN/ONNX：15 分类，合成数据训练（最终形态）
  │
...
|---|---|---|---|---|---|
| **模板匹配**（OpenCV matchTemplate / 特征点） | 近 100% | 中（透视/反光会降准） | ★☆☆ 低 | 极小（几十 KB 模板图） | MVP 首选，截图场景可长期用 |
| **小型 CNN → ONNX**（15 分类） | 极高（>99%） | 高（需含翻拍样本训练） | ★★☆ 中 | 小（量化后 1–5MB） | 翻拍场景的最终形态 |
| **通用 OCR**（PaddleOCR） | — | — | ★★☆ 中 | 重（50–200MB+） | 本项目不需要 |

**建议落地顺序**：第一步用模板匹配跑通整条流水线——因为只有《燕云十六声》一种皮肤，只需截一套该游戏的棋子模板即可，截图场景几乎立刻达到近 100%，验证「截图→FEN→自动布局」闭环。第二步针对手机翻拍场景，用合成数据（把这套棋子贴到棋盘背景上、叠加随机透视/亮度/反光/摩尔纹自动生成成千上万张训练图）+ 少量真实翻拍样本，训练一个小 CNN 导出 ONNX。因为皮肤唯一，CNN 几乎可「程序化无限生成」训练数据，泛化压力极小，这是本任务最大的便利。
...
| 输入类型 | 样例特征 | 难度 | 处理要点 |
|---|---|---|---|
| **游戏截图**（PC/手机原生截图） | 棋盘正对、清晰、底色一致 | 低 | 检测外框→切 90 格→分类；**模板匹配即可近 100%** |
| **手机翻拍屏幕**（拍笔记本/显示器） | 梯形透视、屏幕高光反光、摩尔纹、偏灰、可能有手指遮挡 | 高 | **四角透视校正 (warpPerspective)** → CNN（带透视/高光/摩尔纹/白平衡增强训练）；遮挡格需人工补 |

> 据此最终结论：**截图路用模板匹配/轻量 CNN 即可**；**翻拍路是工作量重心**，必须做透视校正 + 抗高光/摩尔纹的数据增强。两路共用同一套「定位→切格→分类→FEN」骨架，只是定位与增强强度不同。
...
这也是为什么建议「主力用模板匹配/轻量 CNN、PaddleOCR 仅按需」——能把后端体积控制在小工具可以接受的范围内。
...
预估安装包体积：**壳体 + Python 后端 + 模板/模型文件**。在不带 PaddleOCR、不离线内置 WebView2 的前提下，整体大概率能控制在几十 MB 到百 MB 内；实际大小必须以 PyInstaller/Tauri 原型包为准。

> 进一步省体积的选项：截图场景其实只用到 OpenCV 模板匹配，若早期只做截图识别，后端可以不带 ONNX Runtime，包体更小。翻拍场景上线时再加入 ONNX 即可。

---

## 5. 分阶段落地建议

1. **阶段一｜截图识别闭环（不打包）**：先在现有 Web 项目里加一个本地 Python 服务（FastAPI），用 **模板匹配** 跑通「《燕云十六声》截图→FEN→自动布局」。目标是验证棋盘定位、90 格切分、FEN 对接和编辑器回填。
2. **阶段二｜翻拍增强**：如果手机翻拍是高优先级，再做透视校正、反光/摩尔纹/白平衡增强，并用合成数据 + 少量真实翻拍样本训练 15 分类小 CNN，导出 ONNX 替换或补充模板匹配。
3. **阶段三｜桌面打包**：PyInstaller 打包后端为 sidecar，接入 Tauri 2；先出 Windows 包并压测，再补 macOS 包。
4. **阶段四｜分发完善**：处理 Windows 签名/SmartScreen、macOS 签名与公证、自动更新、错误日志收集、模型/模板版本管理。
